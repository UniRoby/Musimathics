...
def getChords(s):
    ...
    if(s=="Do" or s=="do" or s=="c" or s=="C"):
           ...
           chords=[["Dmaj", "D","F#","A"],["Emin","E","G","B"],["F#min","F#","A","C#"],["Gmaj","G","B","D"]]
    elif(s=="Mi" or s=="mi" or s=="e" or s=="E"):
           chords=[["Emaj", "E","G#","B"],["F#min","F#","A","C#"],["G#min","G#","B","D#"],["Amaj","A","C#","E"]]
    elif(s=="Fa" or s=="fa" or s=="f" or s=="F"):
           chords=[["Fmaj","F","A","D"],["Gmin","G","Bb","D"],["Amin","A","C","E"],["Bbmin","Bb","D","F"]]
    elif(s=="Sol" or s=="sol" or s=="g" or s=="G"):
           chords=[["Gmaj","G","B","D"],["Amin","A","C","E"],["Bmin","B","D","F#"],["Cmaj", "C","E","G"]]   
    elif(s=="La" or s=="la" or s=="a" or s=="A"):
           chords=[["Amaj","A","C#","E"],["Bmin","B","D","F#"],["C#min","C#","E","G#"],["Dmaj","D","F#","A"]]
    elif(s=="Si" or s=="si" or s=="b" or s=="B"):
           chords=[["Bmaj", "B","D#","F#"],["C#min","C#","E","G#"],["D#min","D#","F#","A#"],["Emaj","E","G#","B"]]
    return chords
...
def createChordStream(chords,song, num_beats):
    ...
    for i in range(num_beats):
        c = chord.Chord([chords[i%len(chords)][1], chords[i%len(chords)][2], chords[i%len(chords)][3]], quarterLength = 1.0)
        #print(c)
        if(count<4):
            print(c.pitchedCommonName)
            text_msg.insert(tkinter.END, "\n ")
            text_msg.insert(tkinter.END, c.pitchedCommonName)
            count+=1
...
def button_callback():
        ...
        text_msg.insert(tkinter.END, "\nScala di: ")
        text_msg.insert(tkinter.END,segmented_button_1.get())
        text_msg.insert(tkinter.END, "\nStrumento usato: ")
        text_msg.insert(tkinter.END,optionmenu_1.get())
        text_msg.insert(tkinter.END, "\nLunghezza sequenza fibonacci:  ")
        text_msg.insert(tkinter.END,entry_1.get())
        text_msg.insert(tkinter.END, "\nVelocità inserita:  ")
        text_msg.insert(tkinter.END,entry_2.get())
        text_msg.insert(tkinter.END, "  nome: ")
        if(entry_2.get()!=''):
            text_msg.insert(tkinter.END, getVelocità(int(entry_2.get())).text)
        else:
        ...
        scala=sceltaScala(segmented_button_1.get())
        print()
        song=fibonacciComposer(scala,int(entry_1.get()))
        print()
        #chords=getChords(segmented_button_1.get())
        #print("\nAccordi Scala di ",segmented_button_1.get(),chords)
...
root.title("Progetto Musimatica")

sidebar_frame= ctk.CTkFrame(root)
sidebar_frame.pack(pady=10,padx=10,fill="both", side="left")

main_frame= ctk.CTkFrame(root)
main_frame.pack(pady=10,padx=10,fill="both",expand=True,side="top")
...
text_msg.pack(pady=10,padx=10,fill="both",expand=True)
text_msg.insert(tkinter.END, "Benvenuto\n\n\n\n")




logo_label = ctk.CTkLabel(sidebar_frame, text="Progetto Musimatica", font=ctk.CTkFont(size=20, weight="bold"))
logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))
#bottoni

sidebar_button_1 = ctk.CTkButton(sidebar_frame,text="Apri sull' App", command=button_openApp)
sidebar_button_1.grid(row=1, column=0, padx=20, pady=10)
sidebar_button_2 = ctk.CTkButton(sidebar_frame,text="Salva file midi", command=button_saveMidi)
sidebar_button_2.grid(row=2, column=0, padx=20, pady=10)
sidebar_button_3 = ctk.CTkButton(sidebar_frame,text="Partitura Alternativa", command=button_AxAy)
...
label_1 = ctk.CTkLabel(main_frame,text="Compositore di Fibonacci ",justify=tkinter.LEFT,font=("Arial Black",25),anchor=tkinter.CENTER)
label_1.grid(row=1,column=1,pady=10, padx=10)


label_2 = ctk.CTkLabel(main_frame,text="Scegli una scala con cui comporre ",justify=tkinter.LEFT, font=("Helvetica",15))
label_2.grid(row=2,column=0,pady=10, padx=10)
segmented_button_1 = ctk.CTkSegmentedButton(main_frame, values=["Do", "Re","Mi","Fa","Sol","La","Si"])
segmented_button_1.grid(row=2,column=1,pady=10, padx=10)
label_3 = ctk.CTkLabel(main_frame,text="Seleziona uno strumento ",justify=tkinter.LEFT, font=("Helvetica",15))
label_3.grid(row=3,column=0,pady=10, padx=10)
optionmenu_1 = ctk.CTkOptionMenu(main_frame, values=["Piano", 'Violino','Chitarra Acustica','Fisarmonica','Sassofono','Cornamusa','Banjo','Clarinetto','Chitarra Elettrica','Coro'])
optionmenu_1.grid(row=3,column=1,pady=10, padx=10)

label_4 = ctk.CTkLabel(main_frame,text="Inserisci lunghezza composizione ",justify=tkinter.LEFT, font=("Helvetica",15))
label_4.grid(row=4,column=0,pady=10, padx=10)
entry_1 = ctk.CTkEntry(main_frame, placeholder_text="Numero seq fib")
entry_1.grid(row=4,column=1,pady=10, padx=10)

label_5 = ctk.CTkLabel(main_frame,text="Inserisci velocità (bmp)  ",justify=tkinter.LEFT, font=("Helvetica",15))
label_5.grid(row=5,column=0,pady=10, padx=10)
entry_2 = ctk.CTkEntry(main_frame, placeholder_text="120")
entry_2.grid(row=5,column=1,pady=10, padx=10)

label_3 = ctk.CTkLabel(main_frame,text="Seleziona pattern isoritmia ",justify=tkinter.LEFT, font=("Helvetica",15))
label_3.grid(row=6,column=0,pady=10, padx=10)
optionmenu_2 = ctk.CTkOptionMenu(main_frame, values=['1','2','3','4','5','6'])
optionmenu_2.grid(row=6,column=1,pady=10, padx=10)

label_4 = ctk.CTkLabel(main_frame,text="Seleziona intensità volume R ",justify=tkinter.LEFT, font=("Helvetica",15))
label_4.grid(row=7,column=0,pady=10, padx=10)
r_Volume = ctk.CTkOptionMenu(main_frame, values=['p', 'mp','mf','f'])
r_Volume.grid(row=7,column=1,pady=10, padx=10)

label_5= ctk.CTkLabel(main_frame,text="Seleziona intensità volume L ",justify=tkinter.LEFT, font=("Helvetica",15))
label_5.grid(row=8,column=0,pady=10, padx=10)
l_Volume = ctk.CTkOptionMenu(main_frame, values=['p', 'mp','mf','f'])
l_Volume.grid(row=8,column=1,pady=10, padx=10)

button_1 = ctk.CTkButton(main_frame, text="Avvia",height= 50, width=200, command=button_callback)

button_1.grid(row=10,column=1,pady=10, padx=10)
...
optionmenu_1.set("Piano")
optionmenu_2.set("1")
r_Volume.set("mf")
l_Volume.set("mf")
segmented_button_1.set("Do")
